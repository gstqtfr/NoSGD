...
import org.garagescience.deeplearning.nosgd.mlp.BackPropNeuralNetwork
...
class GradientChecker(val numChecks: Int, val accuracy: Int) {
  ...
  def apply(network: BackPropNeuralNetwork, data: DataSet) {
  ...
  def checkGradient(gradients: DoubleMatrix, weights: DoubleMatrix, data: DataSet, checks: Int, accuracy: Int, stepNetwork: (DoubleMatrix) => BackPropNeuralNetwork) {
    val threshold = 1e-5
