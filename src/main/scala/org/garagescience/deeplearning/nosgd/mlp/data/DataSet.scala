package org.garagescience.deeplearning.nosgd.mlp.data
...
class ImageTileDataSet(val inputs: DoubleMatrix,
                       val targets: DoubleMatrix,
                       val numClasses: Int,
                       val imageDimension: Int) extends DataSet {
  def copy(inputs: DoubleMatrix, targets: DoubleMatrix) =
    new ImageTileDataSet(inputs, targets, numClasses, imageDimension)
}
...
object ImageTileDataSet {
  ...
  def apply(directory: String, numClasses: Int, imageDimension: Int): ImageTileDataSet = {
    ...
    // TODO: nifty. it shuffles the data for you. nice ...
